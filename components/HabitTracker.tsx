...
export function HabitTracker({ userId }: { userId: string }) {
  ...
  for (let d = 1; d <= maxDay; d++) {
    ...
    const anyDone =
      ...
      customHabits.some(h => habitLogs[h.id]?.[dk])
  ...
  // Heights for sticky rows (section header ~40px, habit names ~120px)
  const SEC_H = 40
  const HAB_H = 120
      ...
      {addingTo && (
        <div style={{ background: T.surface2, border: `1px solid ${COLORS[addingTo].main}40`, borderRadius: 10, padding: '12px 16px', marginBottom: 16, display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={() => { setAddingTo(null); setNewName('') }}
            style={{ background: 'none', border: 'none', color: T.muted, cursor: 'pointer', fontSize: 20, lineHeight: 1 }}>×</button>
        </div>
      )}

      {/* Grid — scroll container with sticky headers */}
        ...
        <div style={{ overflow: 'auto', maxHeight: 'calc(100vh - 320px)', minHeight: 300 }}>
          <table style={{ borderCollapse: 'collapse', minWidth: '100%' }}>
            <thead>
              ...
              <tr>
                <th style={{
                  width: 72, minWidth: 72, background: T.surface,
                ...
                {SECTIONS.map(sec => {
                  ...
                  // Trading has no spacer column; others do (+1 for the add button)
                  const colSpan = sec.id === 'trading' ? habits.length : habits.length + 1
                  return (
                    <th key={sec.id} colSpan={colSpan}
                      style={{
                        padding: '10px 12px', background: c.dim,
                        borderBottom: `2px solid ${c.main}`,
                        textAlign: 'left', borderRight: `1px solid ${T.border}`,
              ...
              <tr>
                <th style={{
                  width: 72, minWidth: 72, background: T.surface,
                  position: 'sticky', left: 0, top: SEC_H, zIndex: 5,
                  borderBottom: `1px solid ${T.border}`,
                  padding: '8px 12px', fontSize: 10, color: T.muted, fontWeight: 500, letterSpacing: '0.08em', textAlign: 'left',
                }}>
                  DÍA
                </th>
                {SECTIONS.map(sec => {
                  ...
                  return [
                    ...habits.map(h => {
                      ...
                      return (
                        <th key={h.id} style={{
                          width: 44, minWidth: 44, padding: '6px 2px',
                          background: c.dim,
                          borderBottom: `1px solid ${T.border}`,
                          textAlign: 'center', verticalAlign: 'bottom',
                          position: 'sticky', top: SEC_H, zIndex: 4,
                        }}>
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                            ...
                            <div style={{
                              ...
                              whiteSpace: 'nowrap', maxHeight: HAB_H - 30, overflow: 'hidden',
                            }}>{h.name}</div>
                            {sec.id !== 'trading' && (
                    ...
                    // "+" add button column — not for trading
                    sec.id !== 'trading' && (
                      <th key={sec.id + '-add'} style={{
                        width: 34, minWidth: 34, background: c.dim,
                        borderBottom: `1px solid ${T.border}`,
            ...
            <tbody>
              {Array.from({ length: days }, (_, i) => {
                ...
                return (
                  <tr key={day} style={{ background: isToday ? (theme === 'navy' ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)') : 'transparent' }}>
                    {/* Day label — sticky left */}
                    <td style={{
                      position: 'sticky', left: 0, zIndex: 2,
                      background: isToday ? T.surface2 : T.surface,
                      padding: '3px 12px', borderBottom: `1px solid ${T.border}`, whiteSpace: 'nowrap',
                    }}>
                    ...
                    {SECTIONS.map(sec => {
                      ...
                      return [
                        ...habits.map(h => {
                          ...
                          return (
                            <td key={h.id} style={{ padding: '3px 4px', borderBottom: `1px solid ${T.border}`, background: c.dim, textAlign: 'center' }}>
                              {disabled ? (
                                <div style={{ width: 36, height: 36, borderRadius: 6, background: theme === 'navy' ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.03)', margin: '0 auto' }} />
                              ) : (
                                <div
                                  onClick={() => canClick && handleToggle(h.id, dk)}
                                  style={{
                                    width: 36, height: 36, borderRadius: 6, margin: '0 auto',
                                    background: done ? c.main : (theme === 'navy' ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)'),
                                ...
                                >
                                  {done ? (
                                    ...
                                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: theme === 'navy' ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)' }} />
                                  ) : null}
                                </div>
                              )}
                            </td>
                        ...
                        // Spacer column — only for non-trading sections
                        sec.id !== 'trading' && (
